...
final class SearchSectionController: ListSectionController, UISearchBarDelegate, ListScrollDelegate {
    ...
    override func cellForItem(at index: Int) -> UICollectionViewCell {
        guard let cell: SearchCollectionViewCell = collectionContext?.dequeueReusableCell(of: SearchCollectionViewCell.self, for: self, at: index) as? SearchCollectionViewCell else {
    ...
    func listAdapter(_ listAdapter: ListAdapter, didScroll sectionController: ListSectionController) {
        if let searchBar = (collectionContext?.cellForItem(at: 0, sectionController: self) as? SearchCollectionViewCell)?.searchBar {
